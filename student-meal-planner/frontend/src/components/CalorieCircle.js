 ...
 * @param {object} props
 * @param {number} props.calories - 현재 섭취한 칼로리
 * @param {number} props.recommended - 권장 칼로리
 */
const CalorieCircle = ({ calories = 0, recommended = 2500 }) => {
  const radius = 50; // 원의 반지름
  const circumference = 2 * Math.PI * radius; // 원의 둘레
  
  // 섭취 비율 계산 (100%를 넘지 않도록)
  const progress = Math.min(calories / recommended, 1);
  ...
  return (
    <div className="relative flex flex-col items-center justify-center p-6 bg-white rounded-2xl shadow-md">
      ...
      <div className="absolute flex flex-col items-center">
        <span className="text-4xl font-bold text-gray-800">{calories}</span>
